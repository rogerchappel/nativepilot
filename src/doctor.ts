...
export async function doctor(rootInput: string, failOn: string[] = []): Promise<DoctorResult> {
  ...
  const demoState = await readDemoState(root);
...
async function readDemoState(root: string): Promise<string | undefined> {
  const text = await readTextIfExists(path.join(root, 'nativepilot.manifest.json'));
  if (!text) return undefined;
  return (JSON.parse(text) as { demoState?: string }).demoState;
}
...
async function checkManifest(root: string, issues: DoctorIssue[]): Promise<void> {
  const text = await readTextIfExists(path.join(root, 'nativepilot.manifest.json'));
  if (!text) return;
  const manifest = JSON.parse(text) as { generator?: string; preset?: string; demoState?: string; managedPaths?: string[] };
  ...
  if (manifest.demoState && !['installed', 'removed'].includes(manifest.demoState)) issues.push(warn('manifest-mismatch', 'nativepilot.manifest.json has an unknown demo state.', 'nativepilot.manifest.json'));
}
