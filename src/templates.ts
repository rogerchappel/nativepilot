...
export function parseProviders(value: string | undefined): Provider[] {
  const supported = 'openai,anthropic,gemini,local';
  const raw = value?.split(',').map((item) => item.trim()).filter(Boolean) ?? supported.split(',');
  if (raw.length === 0) throw new Error(`Provider list cannot be empty. Supported providers: ${supported}.`);
  const allowed = new Set<Provider>(['openai', 'anthropic', 'gemini', 'local']);
  const providers: Provider[] = [];
  for (const item of raw) {
    if (!allowed.has(item as Provider)) throw new Error(`Unknown provider "${item}". Supported providers: ${supported}.`);
...
export function createFiles(options: CreateOptions): GeneratedFile[] {
  ...
  return [
    ...
    file('tsconfig.json', `{"extends":"expo/tsconfig.base","compilerOptions":{"strict":true,"paths":{"@/*":["./src/*"],"@app/*":["./app/*"]}},"include":["app","src","nativepilot.config.ts","expo-env.d.ts"]}\n`),
...
function envExample(providers: Provider[]): string {
  const defaultProvider = runtimeProvider(providers[0]);
  const lines = ['# Development-only values. Never commit real keys.', `EXPO_PUBLIC_AI_PROVIDER=${defaultProvider}`, `EXPO_PUBLIC_AI_MODEL=${providers[0]}-demo`, 'EXPO_PUBLIC_AI_PROXY_URL=http://localhost:8787/v1'];
...
function agentBrief(name: string): string { return `# Agent brief\n\nGoal: extend ${name} without guessing the architecture.\n\n1. Read AGENTS.md.\n2. Keep AI code under src/ai.\n3. Keep demo deletion safe via \`nativepilot clean-demo\`.\n4. Verify with \`npm test\` and \`npm run nativepilot:doctor\`.\n`; }
function manifestFile(name: string, providers: Provider[]): string { return JSON.stringify({ generator: 'nativepilot', guidanceVersion: '1', preset: 'expo', demoState: 'installed', name, providers, managedPaths: ['app', 'src/ai', 'src/theme', 'src/navigation', 'AGENTS.md', 'docs/SECURITY_MODEL.md'] }, null, 2) + '\n'; }
...
function runtimeProvider(provider: Provider): string { return provider === 'openai' ? 'openai-compatible' : provider; }
function aiProviders(providers: Provider[]): string {
  const defaultProvider = runtimeProvider(providers[0]);
  const developmentKeys = providers.flatMap((provider) => provider === 'local' ? [] : [`process.env.EXPO_PUBLIC_${provider.toUpperCase()}_API_KEY`]);
  return `import type { AIProviderConfig } from './types';\n\nexport const enabledProviders = ${JSON.stringify(providers)} as const;\n\nexport const defaultProviderConfig: AIProviderConfig = {\n  provider: (process.env.EXPO_PUBLIC_AI_PROVIDER as AIProviderConfig['provider']) || '${defaultProvider}',\n  baseURL: process.env.EXPO_PUBLIC_AI_PROXY_URL,\n  model: process.env.EXPO_PUBLIC_AI_MODEL || '${providers[0]}-demo'${developmentKeys.length ? `,\n  developmentApiKey: ${developmentKeys.join(' || ')}` : ''}\n};\n`;
}
